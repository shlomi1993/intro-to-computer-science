...
* Function Operation:   This function reduce size if possible (divide it by 2), reduce the top-
                        index by 1 (because the upper popped from the stack) and return the
                        popped element. Technichally it doesn't free the value of the previously
                        top-indexed member in the memory, but it is okay because if a new member
                        need to be pushed to the stack, it will override the former member and if
                        the memory allocated for the stack need to be freed, it will be done by
